...
cat > /mnt/user-data/outputs/witcorp_enterprise.js << 'ENDOFFILE'
// ============================================================
...
// ============================================================
// FETCH VAULT — ascending by client_name + password show/hide & copy
// ============================================================
...
// ============================================================
// DSC FUNCTIONS — ascending by company_name + DIN column
// ============================================================
...
function renderDSC(data) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="9" class="p-16 text-center">
            <div class="flex flex-col items-center gap-4">
    ...
    data.forEach(d => {
        ...
        // DIN display
        const dinDisplay = d.din
            ? `<span class="px-2 py-1 bg-indigo-50 border border-indigo-200 rounded-lg text-xs font-black text-indigo-700 tracking-wider">${d.din}</span>`
        ...
        tbody.innerHTML += `
            <tr class="border-b border-slate-200 hover:bg-slate-50 transition-all">
                ...
                <td class="p-4 text-center whitespace-nowrap">${dinDisplay}</td>
...
async function saveDSC() {
    ...
    const payload = {
        ...
        din: document.getElementById('dDIN').value.trim(),
    ...
    if (!error) {
        ...
        saveActivity(`${id ? 'Updated' : 'Added'} DSC: ${payload.company_name} | ${payload.client_name} | DIN: ${payload.din || 'N/A'} | Status: ${payload.status}`);
        showToast(`DSC ${id ? 'updated' : 'saved'}: ${payload.company_name}`, 'success');
        await new Promise(r => setTimeout(r, 300));
        await fetchDSC();
        document.getElementById('dEditId').value = "";
        ['dCompany', 'dClient', 'dDIN', 'dExpiry', 'dRemarks'].forEach(i => document.getElementById(i).value = "");
...
function editDSC(d) {
    ...
    document.getElementById('dDIN').value = d.din || '';
...
function exportCSV() {
    ...
    if (currentExportType === "records") {
        ...
        csv = "Company,DIN,Client,Status,Expiry Date,Remarks\n";
        rows.forEach(r => { csv += `"${r.company_name||""}","${r.din||""}","${r.client_name||""}","${r.status||""}","${r.expiry_date||""}","${r.remarks||""}"\n`; });
    }
...
function exportExcel() {
    ...
    if (currentExportType === "records") {
        ...
        csv = "Company,DIN,Client,Status,Expiry Date,Remarks,Updated By\n";
        rows.forEach(r => { csv += `"${r.company_name||""}","${r.din||""}","${r.client_name||""}","${r.status||""}","${r.expiry_date||""}","${r.remarks||""}","${r.updated_by||""}"\n`; });
    }
    const bom = "\uFEFF";
...
function exportPDF() {
    ...
    if (currentExportType === "vault") {
        ...
        tableHead = [["Company", "DIN", "Client", "Status", "Expiry Date", "Remarks"]];
        tableData = rows.map(r => [r.company_name||"", r.din||"", r.client_name||"", r.status||"", r.expiry_date||"", r.remarks||""]);
    } else {
